...
test('admin creates one restricted key, rejects invalid policies, rotates and deletes without leaking', async t => {
  ...
  const config = { proxy: { apiKey: 'fixture-master' }, accounts: [], autoHealthCheck: { enabled: false } };
  ...
  const call = (path, body) => fetch(base + '/agent-lb/api/keys' + path, {
    method: body ? 'POST' : 'GET',
    headers: { authorization: 'Bearer fixture-master', 'content-type': 'application/json' },
    ...(body ? { body: JSON.stringify(body) } : {}),
  });
  const policy = { maxDailyTokens: 10, allowedModels: ['claude-allowed'], allowedProviders: ['anthropic'] };
  const created = await (await call('/create', { name: 'station', ...policy })).json();
  ...
  assert.equal(resolveClientAuth(config.proxy, created.key).entry.maxDailyTokens, 10);
  ...
  assert.equal((await (await call('')).json()).keys[0].allowedProviders[0], 'anthropic');
  for (const invalid of [{ maxDailyTokens: 'Infinity' }, { allowedModels: [1] }, { allowedProviders: ['unknown'] }, { expiresAt: 'invalid' }]) {
  ...
  const updated = await (await call('/create', { name: 'station', ...policy })).json();
  assert.equal(config.proxy.clientKeys.length, 1);
  ...
  assert.equal(resolveClientAuth(config.proxy, rotated.client.key).entry.maxDailyTokens, 10);
  const logs = []; const original = console.log;
  console.log = (...args) => logs.push(args.join(' '));
  let result;
  try { result = await (await call('/delete', { key: rotated.client.key })).text(); }
  ...
  assert.equal(JSON.parse(await readFile(process.env.AGENT_LB_CONFIG, 'utf8')).proxy.clientKeys.length, 0);
});
