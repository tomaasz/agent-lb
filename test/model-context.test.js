...
test("proxy body cap is a byte safety limit, not a token or max_tokens limit", () => {
  ...
  assert.equal(
    resolveMaxBodyBytes({ proxy: { maxBodyBytes: "134217728" } }),
    128 * 1024 * 1024,
  );
...
test("only the explicit Anthropic identity-verification 400 is account-scoped", () => {
  ...
  assert.equal(
    isOAuthIdentityVerificationRequired(
      Buffer.from("Identity verification is required to continue."),
    ),
    true,
  );
  assert.equal(
...
test("identity verification cooldown removes only the affected account from rotation", () => {
  ...
  assert.equal(
    am.unavailableReason(am.accounts[0], "claude-opus-5"),
  ...
  assert.equal(
    am.getActiveAccount(null, "claude-opus-5"),
    null,
    "exhausted-fleet probes must respect identity cooldown",
  );
});
...
test("proxy retries compaction on a healthy OAuth account after identity-verification 400", async () => {
  ...
  const upstreamServer = http.createServer((req, res) => {
    ...
    req.on("end", () => {
      ...
      seen.push({
        ...
        proxyKey: req.headers["x-api-key"],
        body: requestBody,
      });
      res.setHeader("content-type", "application/json");
      if (
        JSON.parse(requestBody).messages[0].content ===
        "ordinary invalid request"
      ) {
        res.writeHead(400);
        res.end(
          JSON.stringify({
  ...
  const am = new AccountManager([
    {
      name: "verify-me",
      type: "oauth",
      accessToken: "oauth-needs-verification",
  ...
  try {
    const requestBody = JSON.stringify({
      ...
      messages: [
        { role: "user", content: "compact the complete conversation" },
      ],
    });
    const response = await fetch(
      `http://127.0.0.1:${proxyServer.address().port}/v1/messages`,
      {
        method: "POST",
        headers: {
    ...
    assert.deepEqual(
      ...
      ["Bearer oauth-needs-verification", "Bearer oauth-healthy"],
    );
    assert.deepEqual(
      seen.map((entry) => entry.proxyKey),
      [undefined, undefined],
      "proxy client key must not reach Anthropic",
    );
    assert.deepEqual(
      seen.map((entry) => JSON.parse(entry.body)),
      [JSON.parse(requestBody), JSON.parse(requestBody)],
    );
    assert.equal(
      am.unavailableReason(am.accounts[0], "claude-opus-5"),
      "identity-verification",
    ...
    const ordinaryResponse = await fetch(
      `http://127.0.0.1:${proxyServer.address().port}/v1/messages`,
      {
        ...
        body: JSON.stringify({
          model: "claude-opus-5",
          messages: [{ role: "user", content: "ordinary invalid request" }],
        }),
      },
    );
...
test("all-identity response ignores unrelated Codex accounts and reports a clear 502", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      new Set(seen),
      new Set(["Bearer claude-a", "Bearer claude-b"]),
    );
  } finally {
    await Promise.all([
      new Promise((resolve) => proxyServer.close(resolve)),
...
test("successful response clears identityVerification cooldown", async () => {
  ...
  try {
    ...
    const r2 = await fetch(
      `http://127.0.0.1:${proxyServer.address().port}/tc-acct/claude-test/v1/messages`,
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
...
test("exhaustedMessage formats clear errors for disabled, partitioned and identity-blocked fleets", () => {
  ...
  assert.match(
    exhaustedMessage(emptyAm, null, 0),
    /No accounts configured in (?:AgentLB|Agent-LB)/,
  );

  // 2. All disabled accounts
  const disabled1 = new AccountManager([{ name: "a", disabled: true }]);
  assert.equal(
    exhaustedMessage(disabled1, null, 0),
    "No account can serve this request: all 1 account is disabled.",
  );

  const disabled2 = new AccountManager([
    { name: "a", disabled: true },
    { name: "b", disabled: true },
  ]);
  assert.equal(
    exhaustedMessage(disabled2, "claude-sonnet-5", 0),
    "No account can serve this request for claude-sonnet-5: all 2 accounts are disabled.",
  );

  // 3. Single eligible vs multiple eligible account grammar
  const singleAm = new AccountManager([{ name: "a" }]);
  assert.equal(
    exhaustedMessage(singleAm, null, 30),
    "No account can serve this request: 1 account is at its quota or rate limit. Quota resets in 30s.",
  );

  const singlePlusDisabled = new AccountManager([
    { name: "a" },
    { name: "b", disabled: true },
  ]);
  assert.equal(
    exhaustedMessage(singlePlusDisabled, null, 30),
    "No account can serve this request: 1 account is (1 more disabled) at its quota or rate limit. Quota resets in 30s.",
  );

  const multiAm = new AccountManager([{ name: "a" }, { name: "b" }]);
  assert.equal(
    exhaustedMessage(multiAm, null, 30),
    "No account can serve this request: all 2 accounts are at their quota or rate limit. Quota resets in 30s.",
  );
  ...
  assert.equal(
    exhaustedMessage(mixedAm, null, 30, "anthropic"),
    "No account can serve this request: 1 account is at its quota or rate limit. Quota resets in 30s.",
  );
  assert.equal(
    exhaustedMessage(mixedAm, null, 30, "codex"),
    "No account can serve this request: 1 account is at its quota or rate limit. Quota resets in 30s.",
  );

  const onlyClaude = new AccountManager([
    { name: "claude-1", provider: "anthropic" },
  ]);
  assert.equal(
    exhaustedMessage(onlyClaude, null, 30, "codex"),
    "No codex accounts configured in AgentLB. Please add a codex account via the Web Dashboard or CLI.",
  );
...
test("eligibility reports identity verification and entitlement cooldown reasons", () => {
  ...
  assert.deepEqual(am.eligibility(0), {
    eligible: false,
    reason: "requires identity verification",
  });
  assert.deepEqual(am.eligibility(1), {
    eligible: false,
    reason: "in OAuth entitlement cooldown",
  });
});

test("prober clears identity verification cooldown upon successful probe", async () => {
  const am = new AccountManager([
    { name: "acct-1", type: "oauth", accessToken: "tok-1" },
  ]);
  am.markIdentityVerificationRequired(0, 300);
  assert.equal(am.unavailableReason(am.accounts[0]), "identity-verification");
...
test("formatAccountStatus and UNAVAILABLE_TEXT clearly distinguish identity verification from active status", () => {
  assert.equal(
    UNAVAILABLE_TEXT["identity-verification"],
    "upstream requires identity verification (action needed in browser)",
  );
  ...
  assert.match(
    formatAccountStatus(activeAcct, Date.now(), paint),
    /\[green\]active\[\/green\]/,
  );
...
test("Claude models endpoint exposes accurate parameters matching documentation", async () => {
  ...
  try {
    const res = await fetch(
      `http://127.0.0.1:${proxyServer.address().port}/v1/models`,
      {
    ...
    assert.ok(
      findModel("claude-haiku-4-5"),
      "claude-haiku-4-5 alias is present",
    );
  } finally {
...
test("normalizeAnthropicModelForOAuth normalizes modern aliases to canonical IDs", () => {
  const normHaiku = normalizeAnthropicModelForOAuth(
    Buffer.from(JSON.stringify({ model: "claude-haiku-4-5" })),
  );
  assert.equal(
    JSON.parse(normHaiku.toString()).model,
    "claude-haiku-4-5-20251001",
  );

  const normOpus55 = normalizeAnthropicModelForOAuth(
    Buffer.from(JSON.stringify({ model: "claude-opus-5.5" })),
  );
  assert.equal(JSON.parse(normOpus55.toString()).model, "claude-opus-5-5");

  const normOpus45 = normalizeAnthropicModelForOAuth(
    Buffer.from(JSON.stringify({ model: "claude-opus-4-5" })),
  );
  assert.equal(
    JSON.parse(normOpus45.toString()).model,
  ...
  assert.equal(
    JSON.parse(normSonnet45.toString()).model,
...
test("resolveTargetModel handles Fable, Mythos, and current Opus 5.5 fallbacks", () => {
  ...
  assert.equal(
    resolveTargetModel("gpt-6-astra", "anthropic"),
    "claude-opus-5-5",
  );
  assert.equal(resolveTargetModel("gpt-6-sol", "anthropic"), "claude-opus-5-5");
  assert.equal(
    resolveTargetModel("gpt-5.3-codex", "anthropic"),
    "claude-sonnet-5",
  );
  assert.equal(
    resolveTargetModel("codex-mini-latest", "anthropic"),
    "claude-haiku-4-5-20251001",
  );
  assert.equal(
...
test("OpenAI and Codex models endpoint exposes accurate parameters matching documentation", async () => {
  const am = new AccountManager([]);
  const proxyServer = createProxyServer(am, {
    upstream: "http://127.0.0.1:9",
    proxy: { apiKey: "test-key", trustLoopback: false, trustTailnet: false },
  ...
  try {
    const res = await fetch(
      `http://127.0.0.1:${proxyServer.address().port}/backend-api/codex/models`,
      {
        headers: { "x-api-key": "test-key" },
      },
    );
    ...
    assert.ok(astra, "gpt-6-astra is present");
    assert.equal(astra.context_window, 1050000);
    assert.equal(astra.max_tokens, 128000);
    assert.equal(astra.latency, "moderate");
    assert.equal(astra.default_reasoning_level, "medium");
    assert.equal(astra.reliable_knowledge_cutoff, "2026-04");
    assert.equal(astra.pricing.input_per_mtok, 10);
    ...
    assert.equal(gpt6Sol.context_window, 1050000);
    assert.equal(gpt6Sol.max_tokens, 128000);
    assert.equal(gpt6Sol.latency, "fast");
    assert.equal(gpt6Sol.pricing.input_per_mtok, 2);
    assert.equal(gpt6Sol.pricing.output_per_mtok, 10);
    assert.ok(
      gpt6Sol.supported_reasoning_levels.some((l) => l.effort === "none"),
    );
    assert.ok(
      gpt6Sol.supported_reasoning_levels.some((l) => l.effort === "max"),
    );

    // GPT-5.6 Sol
    const sol = findModel("gpt-5.6-sol");
    assert.ok(sol, "gpt-5.6-sol is present");
    assert.equal(sol.context_window, 1050000);
    assert.equal(sol.max_tokens, 128000);
    assert.equal(sol.latency, "fast");
    assert.equal(sol.pricing.input_per_mtok, 4);
    assert.equal(sol.pricing.output_per_mtok, 20);

    // GPT-5.6 Terra
    const terra = findModel("gpt-5.6-terra");
    assert.ok(terra, "gpt-5.6-terra is present");
    assert.equal(terra.context_window, 1050000);
    assert.equal(terra.max_tokens, 128000);
    assert.equal(terra.pricing.input_per_mtok, 2);
    assert.equal(terra.pricing.output_per_mtok, 12);

    // GPT-5.6 Luna
    const luna = findModel("gpt-5.6-luna");
    assert.ok(luna, "gpt-5.6-luna is present");
    assert.equal(luna.context_window, 1050000);
    assert.equal(luna.max_tokens, 128000);
    assert.equal(luna.pricing.input_per_mtok, 0.2);
    assert.equal(luna.pricing.output_per_mtok, 1.2);

    // Dedicated Codex models
    const gpt53 = findModel("gpt-5.3-codex");
    assert.ok(gpt53, "gpt-5.3-codex is present");
    assert.equal(gpt53.context_window, 400000);
    assert.equal(gpt53.max_tokens, 128000);
    assert.equal(gpt53.pricing.input_per_mtok, 1.75);
    assert.equal(gpt53.pricing.output_per_mtok, 14);

    const gpt52 = findModel("gpt-5.2-codex");
    assert.ok(gpt52, "gpt-5.2-codex is present");
    assert.equal(gpt52.context_window, 400000);
    assert.equal(gpt52.max_tokens, 128000);

    const miniLatest = findModel("codex-mini-latest");
    assert.ok(miniLatest, "codex-mini-latest is present");
    assert.equal(miniLatest.context_window, 200000);
    assert.equal(miniLatest.max_tokens, 100000);
    assert.equal(miniLatest.pricing.input_per_mtok, 1.5);
    assert.equal(miniLatest.pricing.output_per_mtok, 6);

    // GPT-5.5
    const gpt55 = findModel("gpt-5.5");
    assert.ok(gpt55, "gpt-5.5 is present");
    assert.equal(gpt55.context_window, 1050000);
    assert.equal(gpt55.max_tokens, 128000);
    assert.equal(gpt55.pricing.input_per_mtok, 5);
    assert.equal(gpt55.pricing.output_per_mtok, 30);
  } finally {
    await new Promise((resolve) => proxyServer.close(resolve));
  }
});
