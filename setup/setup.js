...
// Domyślne wartości
let targetUrl =
  process.env.AGENT_LB_URL ||
  process.env.AGENTLB_URL ||
  process.env.CLAUDE_LB_URL ||
  "http://localhost:3456";
let apiKey =
  process.env.AGENT_LB_API_KEY ||
  process.env.AGENTLB_API_KEY ||
  process.env.CLAUDE_LB_API_KEY ||
  process.env.CODEX_LB_API_KEY ||
  keyFromCustomHeaders(process.env.ANTHROPIC_CUSTOM_HEADERS) ||
...
function checkUrlOnce(urlPath, urlStr, key) {
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      ...
      (res) => {
        if (res.statusCode === 200) {
          ...
          reject(
            new Error(
              `Serwer odrzucił klucz API (kod HTTP ${res.statusCode}). Sprawdź poprawność klucza proxy.`,
            ),
          );
        } else {
    ...
    req.on("timeout", () => {
      ...
      reject(
        new Error(
          `Przekroczono limit czasu połączenia z ${urlStr}. Sprawdź, czy serwer działa i maszyna ma połączenie z siecią.`,
        ),
      );
    });
    ...
    req.on("error", (err) => {
      reject(
        new Error(
          `Błąd połączenia z ${urlStr} (${err.message}). Upewnij się, że masz połączenie z adresem serwera.`,
        ),
      );
    });
...
function backupPath(filePath) {
  ...
  try {
    ...
    console.warn(
      `[Ostrzeżenie] Nie udało się utworzyć kopii ${filePath}: ${err.message}`,
    );
    return null;
  }
}
...
function removeMarkedSource(rcPath) {
  ...
  if (cleaned !== text)
    writeTextAtomic(rcPath, cleaned, fs.statSync(rcPath).mode & 0o777);
}
...
function uninstallClientSettings() {
  ...
  if (vs) {
    if (Array.isArray(vs["claudeCode.environmentVariables"])) {
      let envVars = vs["claudeCode.environmentVariables"].filter(
        (e) =>
          e.name !== "ANTHROPIC_BASE_URL" &&
          e.name !== "ANTHROPIC_API_KEY" &&
          e.name !== "ANTHROPIC_AUTH_TOKEN",
      );
      const customHdr = envVars.find(
  ...
  if (fs.existsSync(codexTomlPath)) {
    try {
      ...
      console.warn(
        `[Ostrzeżenie] Nie udało się wyczyścić ~/.codex/config.toml: ${err.message}`,
      );
    }
  }
  for (const file of [
    path.join(home, ".config", "agent-lb.env"),
    path.join(home, ".config", "claude-lb.env"),
  ]) {
    try {
      if (fs.existsSync(file)) fs.unlinkSync(file);
    } catch (err) {
      console.warn(
        `[Ostrzeżenie] Nie udało się usunąć ${file}: ${err.message}`,
      );
    }
  }
  for (const rc of [".bashrc", ".zshrc", ".profile"])
    removeMarkedSource(path.join(home, rc));
  if (isWin) {
    try {
      execFileSync("powershell.exe", [
        "-NoProfile",
        "-Command",
        "'ANTHROPIC_BASE_URL','ANTHROPIC_API_KEY','ANTHROPIC_CUSTOM_HEADERS','CODEX_LB_API_KEY','CODEX_BASE_URL','OPENAI_BASE_URL','OPENAI_API_KEY','AGENT_LB_API_KEY' | ForEach-Object { [Environment]::SetEnvironmentVariable($_, $null, 'User') }",
      ]);
    } catch (err) {
      console.warn(
        `[Ostrzeżenie] Nie udało się usunąć zmiennych Windows: ${err.message}`,
      );
    }
  }
  console.log(
...
function safeReadJson(filePath) {
  ...
  try {
    ...
    console.warn(
      `[Ostrzeżenie] Nie można sparsować ${filePath}: ${err.message}. Zostanie utworzona kopia zapasowa.`,
    );
    return null;
  }
}
...
async function main() {
  ...
  if (isWsl) {
    console.warn(
      "[Ostrzeżenie] Wykryto WSL. Ten proces zapisuje konfigurację w linuksowym profilu WSL, nie w C:\\Users\\... Windows. W PowerShell uruchom setup.ps1 bez potoku do bash.",
    );
  }
  ...
  if (!apiKey) {
    ...
    const savedKey =
      existingClaudeSettings?.env?.ANTHROPIC_API_KEY ||
      keyFromCustomHeaders(
        existingClaudeSettings?.env?.ANTHROPIC_CUSTOM_HEADERS,
      );
    if (savedKey) {
      ...
      if (fs.existsSync(p)) {
        try {
          ...
          const match = content.match(
            /export (?:CODEX_LB_API_KEY|ANTHROPIC_API_KEY)=["']?([^"'\r\n]+)/,
          );
          if (match && match[1]) {
  ...
  process.stdout.write(
    `Sprawdzam połączenie i poprawność klucza na ${targetUrl}... `,
  );
  try {
  ...
  try {
    ...
    if (oauthSession) {
      ...
      claudeSettings.env.ANTHROPIC_CUSTOM_HEADERS = proxyCustomHeaders(
        apiKey,
        claudeSettings.env.ANTHROPIC_CUSTOM_HEADERS,
      );
    } else {
    ...
    console.warn(
      `[Ostrzeżenie] Nie udało się zaktualizować ~/.claude/settings.json: ${err.message}`,
    );
  }
  ...
  if (!skipVscode) {
    ...
    for (const dir of vscodeDirs) {
      if (
        fs.existsSync(dir) ||
        (dir.includes(".vscode-server") &&
          fs.existsSync(path.dirname(path.dirname(dir))))
      ) {
        try {
          ...
          if (oauthSession) {
            ...
            const existingHdr = envVars.find(
              (e) => e.name === "ANTHROPIC_CUSTOM_HEADERS",
            )?.value;
            ...
            if (rem)
              envVars.push({ name: "ANTHROPIC_CUSTOM_HEADERS", value: rem });
          }
          vsSettings["claudeCode.environmentVariables"] = envVars;

          writeJsonSafe(vscodeSettingsFile, vsSettings, 0o600);
          console.log(
            `[OK] Skonfigurowano oficjalne rozszerzenie Claude Code w VS Code (${vscodeSettingsFile}).`,
          );
          configuredAny = true;
        } catch (err) {
          console.warn(
            `[Ostrzeżenie] Nie udało się zaktualizować VS Code w ${dir}: ${err.message}`,
          );
        }
      }
    }
    if (!configuredAny) {
      console.log(
        `[INFO] Nie wykryto katalogu VS Code (${vscodeSettingsDir}) — pominięto konfigurację rozszerzenia IDE.`,
      );
    }
  }

  // 5b. Konfiguracja OpenAI Codex CLI (~/.codex/config.json oraz config.toml)
  const codexDir = path.join(os.homedir(), ".codex");
  if (setupCodex || fs.existsSync(codexDir)) {
    const codexConfigFile = path.join(codexDir, "config.json");
    const codexTomlFile = path.join(codexDir, "config.toml");
    try {
      ...
      console.warn(
        `[Ostrzeżenie] Nie udało się zaktualizować konfiguracji Codex CLI: ${err.message}`,
      );
    }
  }
  ...
  if (!skipEnv) {
    if (isWin) {
      try {
        ...
        let winCmd =
          `[Environment]::SetEnvironmentVariable('ANTHROPIC_BASE_URL', '${psQuote(targetUrl)}', 'User'); ` +
          (oauthSession
        ...
        console.warn(
          `[Ostrzeżenie] Nie udało się ustawić zmiennych w Windows: ${err.message}`,
        );
      }
    } else {
      // Linux / macOS
      ...
      try {
        ...
        for (const rcName of [".bashrc", ".zshrc"]) {
          ...
          if (fs.existsSync(rcPath)) {
            ...
            if (
              !rcContent.includes("# agent-lb") &&
              !rcContent.includes("# claude-lb")
            ) {
              backupPath(rcPath);
              writeTextAtomic(
                rcPath,
                `${rcContent}${rcContent.endsWith("\n") ? "" : "\n"}${srcLine}\n`,
        ...
        console.warn(
          `[Ostrzeżenie] Nie udało się zapisać zmiennych powłoki: ${err.message}`,
        );
      }
    }
  }
  ...
  if (runTest) {
    ...
    try {
      ...
      if (oauthSession) {
        ...
        testEnv.ANTHROPIC_CUSTOM_HEADERS = proxyCustomHeaders(
          apiKey,
          testEnv.ANTHROPIC_CUSTOM_HEADERS,
        );
      } else {
      ...
      console.warn(
        `[Test] Uwaga: polecenie 'claude' zwróciło błąd lub nie jest w PATH: ${err.message}`,
      );
    }
  }
  ...
  console.log(
    "1. Claude Code CLI: będzie automatycznie łączyć się przez AgentLB w każdym terminalu.",
  );
  console.log(
    "2. Rozszerzenie VS Code: po zrestartowaniu okna VS Code będzie używać Twojego proxy i klucza.",
  );
  console.log(
    "3. Inne narzędzia (Cursor, Cline, Continue, Roo Code): jako Base URL ustaw " +
      targetUrl +
      ", a jako klucz swój tc-...",
  );
  if (setupCodex || fs.existsSync(codexDir)) {
    console.log(
