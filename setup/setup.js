...
function uninstallClientSettings() {
  ...
  if (isWin) {
    try {
      execFileSync('powershell.exe', ['-NoProfile', '-Command', "'ANTHROPIC_BASE_URL','ANTHROPIC_API_KEY','CODEX_LB_API_KEY','CODEX_BASE_URL','OPENAI_BASE_URL' | ForEach-Object { [Environment]::SetEnvironmentVariable($_, $null, 'User') }"]);
