// Run the proxy as a user service — `agentlb service install`.
//
...
// same way `agentlb alias --install` handles the shell alias.
//
...
export const LABEL = 'com.agentlb.proxy';
export const UNIT_NAME = 'agentlb.service';
...
export function logPath(home = homedir(), platform = process.platform) {
  return platform === 'darwin'
    ? join(home, 'Library', 'Logs', 'agentlb.log')
    : join(home, '.local', 'state', 'agentlb.log');
}
...
//     as `agentlb status` timeouts and multi-second event-loop lag while the
...
// plist is only regenerated by `agentlb service install`.
export function renderLaunchAgent({ node, entry, log, path, configPath = null }) {
  ...
  if (configPath) env.push(`    <key>AGENT_LB_CONFIG</key>\n    <string>${xmlEscape(configPath)}</string>`);
 ...
 * newline anywhere started a new line of the unit — so a AGENT_LB_CONFIG
...
export async function installService({
  ...
  if (kind === 'launchd') {
    ...
    log(`[AgentLB] Service installed: ${plist}`);
    log(`[AgentLB] Logs: ${logFile}`);
  ...
  log(`[AgentLB] Service installed: ${unit}`);
  log('[AgentLB] Logs: journalctl --user --unit agentlb.service --follow');
  // Without lingering the unit dies with the last login session, which is not
  // what "install a service" is expected to mean.
  log('[AgentLB] To keep it running with no session open: loginctl enable-linger $USER');
...
export async function uninstallService({
  ...
  if (kind === 'launchd') {
    ...
    log(`[AgentLB] Service removed: ${plist}`);
  ...
  run('systemctl', ['--user', 'daemon-reload']);
  log(`[AgentLB] Service removed: ${unit}`);
