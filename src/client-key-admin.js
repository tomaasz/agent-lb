...
export async function handleClientKeys({
  ...
  if (
    req.method === "GET" &&
    (normApiPath === "/api/keys" || normApiPath === "/client-keys")
  ) {
    const clientsStats =
      clientUsage?.export() || hooks.getStatusExtra?.()?.clients || {};
    const keys = (config.proxy?.clientKeys || []).map((k) => {
      const raw = k.key || "";
      const masked = maskSecret(raw);
      const stat = clientsStats[k.name] || {
        requests: 0,
        connections: 0,
        inputTokens: 0,
        outputTokens: 0,
      ...
      return {
        ...
        maxDailyTokens:
          k.maxDailyTokens != null ? Number(k.maxDailyTokens) : null,
        maxMonthlyTokens:
          k.maxMonthlyTokens != null ? Number(k.maxMonthlyTokens) : null,
        expiresAt: k.expiresAt || null,
        allowedModels: Array.isArray(k.allowedModels) ? k.allowedModels : null,
        allowedProviders: Array.isArray(k.allowedProviders)
          ? k.allowedProviders
          : null,
  ...
  // Client Keys: Reveal unmasked keys for administrative deployment (POST /agentlb/api/keys/reveal & POST /agentlb/client-keys/reveal)
  if (
    req.method === "POST" &&
    (normApiPath === "/api/keys/reveal" ||
      normApiPath === "/client-keys/reveal")
  ) {
    let body;
    try {
    ...
    if (body?.all || !name) {
      ...
      for (const k of config.proxy?.clientKeys || []) {
        if (k.name && k.key) keyMap[k.name] = k.key;
      }
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(
        JSON.stringify({
          ok: true,
          keys: keyMap,
          primaryKey: config.proxy?.apiKey || "",
        }),
      );
      return true;
    }

    if (name === "__primary__" || name === "primary") {
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(
        JSON.stringify({
          ok: true,
    ...
    const found = (config.proxy?.clientKeys || []).find((k) => k.name === name);
    if (!found) {
      res.writeHead(404, { "Content-Type": "application/json" });
      res.end(
        JSON.stringify({ ok: false, error: "workstation key not found" }),
      );
      return true;
    }
  ...
  if (
    req.method === "POST" &&
    (normApiPath === "/api/keys/create" ||
      normApiPath === "/client-keys/add" ||
      normApiPath === "/client-keys")
  ) {
    let body;
    try {
      const raw = await readControlBody(req);
    ...
    for (const field of ["maxDailyTokens", "maxMonthlyTokens"]) {
      ...
      if (!Number.isSafeInteger(value) || value <= 0) {
        res.writeHead(400);
        res.end(JSON.stringify({ ok: false, error: "invalid token limit" }));
        return true;
      }
      policy[field] = value;
    }
    if (body.expiresAt != null) {
      if (
        ...
        res.writeHead(400);
        res.end(JSON.stringify({ ok: false, error: "invalid expiry" }));
        return true;
      }
        ...
        body[field].some(
          (v) =>
            ...
            (field === "allowedProviders" &&
              !["anthropic", "codex"].includes(v)),
        )
      ) {
        res.writeHead(400);
        res.end(JSON.stringify({ ok: false, error: "invalid access policy" }));
        return true;
      }
    ...
    const updated = await atomicConfigUpdate((disk) => {
      ...
      if (
        key === disk.proxy.apiKey ||
        disk.proxy.clientKeys.some((k) => k.name !== name && k.key === key)
      ) {
        duplicate = true;
        return;
      }
      const existing = disk.proxy.clientKeys.find((k) => k.name === name);
      disk.proxy.clientKeys = disk.proxy.clientKeys.filter(
        (k) => k.name !== name,
      );
      disk.proxy.clientKeys.push({
        name,
        key,
    ...
    if (duplicate) {
      res.writeHead(409);
      res.end(JSON.stringify({ ok: false, error: "key already in use" }));
      return true;
    }
    config.proxy ||= {};
  ...
  if (
    (req.method === "POST" &&
      (normApiPath === "/api/keys/delete" ||
        normApiPath === "/client-keys/remove")) ||
    (req.method === "DELETE" &&
      (normApiPath === "/client-keys" || normApiPath === "/api/keys"))
  ) {
    let body;
    try {
      const raw = await readControlBody(req);
    ...
    await atomicConfigUpdate((disk) => {
      if (Array.isArray(disk.proxy?.clientKeys)) {
        disk.proxy.clientKeys = disk.proxy.clientKeys.filter(
          (k) => k.name !== target && k.key !== target,
        );
      }
    });

    if (Array.isArray(config.proxy?.clientKeys)) {
      config.proxy.clientKeys = config.proxy.clientKeys.filter(
        (k) => k.name !== target && k.key !== target,
      );
    }
  ...
  if (
    req.method === "POST" &&
    (normApiPath === "/client-keys/rotate" ||
      normApiPath === "/api/keys/rotate")
  ) {
    let body;
    try {
      const raw = await readControlBody(req);
