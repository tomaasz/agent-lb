...
// statements for `eval "$(agentlb env)"` — a name like "work (Acme)" would be
...
// environment `agentlb run` sets up, but emitted for `eval "$(agentlb
...
//
// MITM (forward-proxy) mode is the default, matching `agentlb run`: it routes
...
// `account` pins the session to one account (TC_ACCT), exactly as `agentlb
